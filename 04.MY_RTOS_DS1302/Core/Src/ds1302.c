...
uint8_t read_ds1302(uint8_t addr)
{
	...
	*(unsigned int *)GPIOA_ODR |= CE_DS1302_Pin;
	// 2. addr 전송
...
void rx_ds1302(unsigned char *pdata)
{
	...
	for (int i=0; i<8; i++)
	{
		...
		if (*(unsigned int*)GPIOA_IDR & IO_DS1302_Pin)
		{
			// 1의 조건만 set
			temp |= 1 << i;
		}
...
void tx_ds1302(uint8_t value)
{
	...
	for (int i=0; i<8; i++)
	{
		if (value & (1 << i))
		{
			...
			*(unsigned int *)GPIOA_ODR |= IO_DS1302_Pin;
		}
		else
		{
...
//	HAL_GPIO_Init(GPIOA, &GPIO_init);		// 싱글변수이기 때문에 &로 주소를 넘김
	*(unsigned int*)GPIOA_MODER |= 0x00400000;
	*(unsigned int*)GPIOA_MODER &= ~0x00800000;
	*(unsigned int*)GPIOA_PUPDR &= ~0x00C00000;
	*(unsigned int*)GPIOA_OSPEEDR |= 0x00C00000;
}
...
//	GPIO_init.Pull = GPIO_NOPULL;
//	HAL_GPIO_Init(GPIOA, &GPIO_init);		// 싱글변수이기 때문에 &로 주소를 넘김
	*(unsigned int*)GPIOA_MODER &= ~0x00C00000;
	*(unsigned int*)GPIOA_PUPDR &= ~0x00C00000;
}
...
void clock_ds1302()
{
	...
	*(unsigned int *)GPIOA_ODR |= CLK_DS1302_Pin;
